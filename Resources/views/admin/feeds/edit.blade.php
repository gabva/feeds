...
@section('content-header')
    <h1>
        {{ trans('feeds::feeds.title.edit feed') }} <small>{{$feed->name}} </small>
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ URL::route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ URL::route('admin.feeds.feed.index') }}">{{ trans('feeds::feeds.title.feeds') }}</a></li>
        <li class="active">{{ trans('feeds::feeds.title.edit feed') }}</li>
    </ol>
@stop
...
@section('content')
    {!! Form::model($feed,['route' => ['admin.feeds.feed.update',$feed->id], 'method' => 'put']) !!}
